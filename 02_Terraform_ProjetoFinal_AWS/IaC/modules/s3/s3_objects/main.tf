...
resource "aws_s3_object" "python_scripts" {
  for_each = fileset("${var.files_bucket}/", "**") # Loop que vai percorrer tudo o que tem na pasta pipeline, criando um fileset para colocar na pasta do s3
  bucket = var.name_bucket
  key    = "pipeline/${each.value}"
  source = "${var.files_bucket}/${each.value}"
  etag   = filemd5("${var.files_bucket}/${each.value}") # filemd5: algoritmo hash que valida a integridade de um arquivo - camada adicional de segurança
}
...
# output
resource "aws_s3_object" "output" { # pasta de saída do processamento
  bucket = var.name_bucket
  key    = "output/"
}
