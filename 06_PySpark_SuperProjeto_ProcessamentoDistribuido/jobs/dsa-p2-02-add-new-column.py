...
if 'percentual_bonus' not in df.columns:
    ...
    # a função .lit cria um valor constante, ou seja, todas as linhas terão o percentual_bonus no valor 0.3
    df.withColumn("percentual_bonus", lit(0.3)).show()
    
# Adiciona uma coluna com o valor do bônus calculado com base no salário
# ATENÇÃO: A função .withColumn adiciona uma coluna apenas em tempo de execução. Não salva na tabela.
# Se quiser salvar na tabela, precisa salvar o data frame (lembrando que isso consome memória)
...
# Aqui, basicamente é uma condição if, mas não estamos usando if pois o if é python puro.
# Num caso de clusters com processamento distribuído, não é recomendado usar python puro.
# Assim, a sintaxe correta para lidar com condicionais em processamento distribuído em pyspark é usar WHEN:
df.withColumn("nivel", \
...
# Mostra o DataFrame
# Quando vc exibe o data frame, vai observar que ele continua INTACTO. Todas as operações e colunas que adicionamos acima
# não foram gravadas, conforme comentário lá em cima, pois todas as operações estão sendo feitas em tempo de execução,
# usando a memória do cluster spark.
df.show()
